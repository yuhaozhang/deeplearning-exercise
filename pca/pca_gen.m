...
eigen_v = diag(S);
sum_v = sum(eigen_v);
for k=1:length(eigen_v)
    if sum(eigen_v(1:k)) / sum_v >= 0.99
        break
    end
end
...
%%% YOUR CODE HERE %%%
xTilde = U(:, 1:k)' * x; % with PCA dimension reduction
xHat = U(:, 1:k) * xTilde; % reconstructed full-dimension matrix
...
epsilon = 1e-1; 
%%% YOUR CODE HERE %%%
xPCAWhite = diag(1 ./ sqrt(eigen_v + epsilon)) * U' * x;
